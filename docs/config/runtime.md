...
Out of the box, Distillery also includes a provider for `Config`, which
allows you to use `config.exs` files with a release. You have to opt in to
this provider, because `Config` blends compile-time and runtime
...
configuration using `Config`; create a dedicated config which handles
